                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
                    <View style={[s.contentWrapper, isWeb && { width: contentWidth, alignSelf: 'center' }]}>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.hList}>
                            {(filteredOffers.length > 0 ? filteredOffers : activeOffers).slice(0, 6).map((item) => (
                                <TouchableOpacity 
                                    key={item._id || item.id} 
                                    style={[s.recCard, { width: isWeb ? 300 : width * 0.7 }]}
                                    onPress={() => navigation.navigate('OfferDetails', { offerId: item._id || item.id })}
                                >
                                    <View style={s.recImgWrapper}>
                                        <Image source={{ uri: item.image || 'https://via.placeholder.com/200' }} style={s.recImg} />
                                        <View style={s.cardBadge}><Text style={s.cardBadgeTxt}>LATEST</Text></View>
                                    </View>
                                    <View style={s.recInfo}>
                                        <Text style={s.recTitle} numberOfLines={1}>{item.title}</Text>
                                        <Text style={s.recSub}>{item.storeId?.storeName || 'Boutique'}</Text>
                                        <View style={s.recPriceRow}>
                                            <Text style={s.recPrice}>₹{(item.originalPrice * (1 - item.discount / 100)).toLocaleString()}</Text>
...
const s = StyleSheet.create({
    ...
    logoTxt: { color: '#D4AF37', fontSize: 18, fontWeight: '900' },
    ...
    searchBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 14, paddingHorizontal: 16, height: 50, borderWidth: 1, borderColor: 'rgba(212,175,55,0.1)' },
    searchInput: { flex: 1, color: '#fff', fontSize: 14 },
    mapCard: { marginHorizontal: 24, marginTop: 10, height: 160, borderRadius: 24, overflow: 'hidden' },
    mapBg: { flex: 1 },
    mapOverlay: { flex: 1, padding: 24, justifyContent: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 20 },
    ...
    mapSub: { color: '#A0A0B0', fontSize: 13, marginTop: 2 },
    ...
    sectionTitleAlt: { color: '#fff', fontSize: 22, fontWeight: '900', paddingHorizontal: 24, marginTop: 32, marginBottom: 16 },
    viewAll: { color: '#D4AF37', fontWeight: '700', fontSize: 14 },
    hList: { paddingHorizontal: 24, gap: 16 },
    recCard: { backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 24, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', marginRight: 16 },
    recImgWrapper: { width: '100%', height: 160, position: 'relative' },
    recImg: { width: '100%', height: '100%', resizeMode: 'cover' },
    cardBadge: { position: 'absolute', top: 12, left: 12, backgroundColor: '#D4AF37', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
    cardBadgeTxt: { color: '#000', fontSize: 9, fontWeight: '950' },
    recInfo: { padding: 16 },
    recTitle: { color: '#fff', fontSize: 16, fontWeight: '800' },
    recSub: { color: '#8E8E93', fontSize: 12, marginTop: 4 },
    recPriceRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 12 },
    recPrice: { color: '#D4AF37', fontSize: 18, fontWeight: '900' },
    ...
    catBtn: { width: (width - 60) / 2, height: 110, backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 20, alignItems: 'center', justifyContent: 'center', gap: 10, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
    catBtnActive: { backgroundColor: 'rgba(212,175,55,0.15)', borderColor: '#D4AF37' },
    catIconBox: { width: 44, height: 44, borderRadius: 14, backgroundColor: 'rgba(212,175,55,0.1)', alignItems: 'center', justifyContent: 'center' },
    catLabel: { color: '#8E8E93', fontSize: 14, fontWeight: '700' },
    catLabelActive: { color: '#fff' },
    recentList: { paddingHorizontal: 24, gap: 12 },
    recentCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 20, padding: 12, gap: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
    recentImg: { width: 56, height: 56, borderRadius: 14, backgroundColor: '#111' },
    recentInfo: { flex: 1 },
    recentTitle: { color: '#fff', fontSize: 15, fontWeight: '700' },
    recentSub: { color: '#8E8E93', fontSize: 12, marginTop: 4, fontStyle: 'italic' },
});
