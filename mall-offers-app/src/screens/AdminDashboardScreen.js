    ...
    return (
        <View style={s.container}>
            <LinearGradient colors={['#0f0c29', '#302b63', '#24243e']} style={s.gradient}>
                ...
                {activeTab === 'approvals' && (
                    <FlatList data={pendingStores} keyExtractor={i => i.id} contentContainerStyle={s.list} showsVerticalScrollIndicator={false}
                        renderItem={({ item }) => (
                            <View style={s.card}>
                                ...
                                <View style={{ flexDirection: 'row', gap: 10 }}>
                                    <TouchableOpacity style={{ flex: 1 }} onPress={() => handleApprove(item.id)}>
                                    ...
                                    <TouchableOpacity style={{ flex: 1 }} onPress={() => handleReject(item.id)}>
                ...
                {activeTab === 'users' && (
                    <FlatList data={users} keyExtractor={i => i.id} contentContainerStyle={s.list} showsVerticalScrollIndicator={false}
                        renderItem={({ item }) => {
                                ...
                                <View style={s.card}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                        ...
                                        <View style={{ alignItems: 'flex-end', gap: 8 }}>
                                            ...
                                            {item.role !== 'admin' && <TouchableOpacity onPress={() => handleDeleteUser(item.id)}><Ionicons name="trash-outline" size={18} color="#FF6B6B" /></TouchableOpacity>}
                                        </View>
                                    </View>
                                </View>
                            );
                ...
                {activeTab === 'offers' && (
                    <FlatList data={offers} keyExtractor={i => i.id} contentContainerStyle={s.list} showsVerticalScrollIndicator={false}
                        renderItem={({ item }) => (
                            <View style={s.card}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <View style={{ flex: 1 }}>
                                    ...
                                    <TouchableOpacity style={s.delBtn} onPress={() => handleDeleteOffer(item.id)}>
